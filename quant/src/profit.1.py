...
def cal_profit(ob, pre, obj,lastobj):
    ...
    if preBuy > ob[buy, 0]:
        ...
        print ("want1  %f" % want)
    #else:
    #    want = max(preBuy, ob[buy, 0] * 0.99)
    if pre[buy, 4] > ob[buy, 0]:
        want  = min(want, ob[buy, 0] * 0.99)
        print ("want2  %f" % want)
    #else:
    #    want = min(want, max(pre[buy, 4] ,ob[buy,0] * 0.99))
    start = min ( want , ob[buy,0] )
    # start = min(preBuy, ob[buy,0] * p)
    #target = start * (min (0.2, preProfit / 2 ))
    target = start * ( 1 + min (0.3, preProfit ))
    ob_low = ob[buy,3] 
        ...
        #     break
            ...
            if i != buy:
                ...
                if ob[i, 2] > target:
                    ...
                    break
                # out of high
                new_target = target + (target - start * 1.01)/(sell - 10) *(i - sell)
                #if i > buy and ob[i, 2] > new_target:
                if i > sell :
                    end = max(ob[i,0], new_target)
                    print ("new target terminal at : e=%f, i=%d , t=%f   new target %f "%(e, i, target , new_target))
                    break
                # get low bond 
                try:
                    #print ("ob:%f, start:%f" %(ob[i, buy], start * 0.98))
                    pass
                except:
                    print (ob)
                    print ("ob:%f, start:%f" %(ob[buy, i], start * 0.98))
                if ob[i,3] < start * 0.98 and e > 0.3:
                    end = ob[i,1]
                    print ("2terminal out loss : sold=%f, i=%d "%(ob[i,1], i))
                    break
                if ob[i,3] < start * 0.9:
                    end = ob[i,1]
                    print ("3terminal out loss : sold=%f, i=%d "%(ob[i,1], i))
                    break
            elif i >= sell:
                end = ob[i,1]
    ...
    with open('back') as file:
        ...
        for s in data:
            if '' != s:
                ...
                target_file = 'result/new2compare.csv'
                #target_file = 'result/new1compare.csv'
